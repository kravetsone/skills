...
| ----- | ------- | ----------- |
| [`coolify-monorepo`](./coolify-monorepo/SKILL.md) | 4.1.2 | Deploying & maintaining pnpm-workspace + Turborepo monorepos (`apps/` + `packages/`) on self-hosted [Coolify](https://coolify.io) v4. Source-verified (file:line) build-context model — **Base Directory IS the `docker build` context, Dockerfile Location resolves relative to it** — watch-paths glob grammar (`!` negation, last-match-wins, 20-commit GitHub payload truncation), one-push→N-apps webhook fan-out, GitHub App vs deploy keys, build-pack tradeoffs (Dockerfile / Nixpacks-maintenance-mode / Railpack-beta / Docker Compose w/ shared-env caveat), `turbo prune --docker` multi-stage templates (NestJS, Bun, tsx runtime), all four cache layers (same-SHA build skip, layer cache, BuildKit GC `daemon.json`, Coolify automated cleanup) + turbo remote cache self-hosted on Coolify, dependency-graph hygiene, zero-downtime migration runbook, symptom→cause→fix table with issue links. Pairs with the official `vercel/turborepo` skill. |
