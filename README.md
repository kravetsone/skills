...
| [`adminjs`](./adminjs/SKILL.md) | 2026.4.22 | AdminJS 7 on the Bun + Elysia + Drizzle ORM + S3 stack. Documents `adminjs-elysia` (including the v0.1.4 cookie-name bug and ignored options), `adminjs-drizzle` adapters with their snake_case param + boolean-as-string quirks, `@adminjs/upload`'s `BaseProvider` contract for Web API Blobs (not formidable), multiple upload features per resource, richtext patch, React 18 pin, `.adminjs/` bundle strategy, custom actions & components, JWT auth, plus ready-to-paste templates and `doctor.mjs` / `bundle-check.mjs` / `scaffold-resource.mjs` node scripts. |
